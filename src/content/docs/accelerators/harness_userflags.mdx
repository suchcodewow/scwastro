...
This is an application showing screens for many users at once. The goal was to provide a real-world example making it easy to understand Harness Feature Management and Experimentation (FME) progressive feature release and dynamic configuration.

The scenario is that we work for GlobalCorp. Our job is to implement the new **OmniView** feature. It allows our users to see all of their content in a single screen. But we're worried a bout the performance imnpact (even though the development team ensures us it will work fine). Using Harness FME, we'll use a progressive rollout for this feature and monitor it.
...
First, we'll configure Harness FME to use our application, and the run the application with SDK key connecting to Harness.

## Harness FME

These steps configure FME for a new project with 2 attributes. The attributes are built into the application so can safely be targets for feature flags without exposing any sensitive data.
