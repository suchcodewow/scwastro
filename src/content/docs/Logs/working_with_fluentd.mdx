...
Type :wq _enter_ to save and exit.
...
![Prometheus_1](/src/assets/img/nginx-to-fluentd.png)
...
The `-f` command will follow the logs until you press _ctrl-c_ to quit.
...
Most of the lines will have the same type of data. But it's easy to confirm it worked if your _responsetime_ fields now have decimals i.e. `"0.027"`. That means fluentd extracted the responsetime value, changed it to a numeric (integer) format and exported it.
...
Please take a moment to mark the spreadsheet _fluentd - Extract & Improve Data_ column for your row with `done`.
...
- _inside_ the metric object for a specific label related to that metric
- _outside_ the metric object for common labels for all the metric defined in our pipeline
...
Please take a moment to mark the spreadsheet _fluentd - Created Prometheus filter_ column for your row with `done`.
...
![FluentD_1](/src/assets/img/fluentd_ds.png)
...
![FluentD_2](/src/assets/img/fluentd_port.png)
...
![fluentd_1_1](/src/assets/img/dt_fluentd_metrics.png)
...
![fluentd_1_2](/src/assets/img/dtu_fluentd_metrics_status.png)

### :checkered_flag: Checkpoint

Please take a moment to mark the spreadsheet _Fluentd - created Graphs_ column for your row with `done`.
