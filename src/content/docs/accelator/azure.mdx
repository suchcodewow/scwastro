...
![choose powershell](/src/assets/img/choose_powershell.jpg)  
...
![azure ps](/src/assets/img/powershell.png)
...
<details>
  <summary>Waiter, there seems to be a cat in my scp?</summary>SCP is the secure copy command. We're using it to copy
  the myip file to your host to use later. The cat command outputs the file contents (just the ip address in this case)
  and the $() part means "run whatever command is in here". In the end, scp gets the azure user name and the IP of the
